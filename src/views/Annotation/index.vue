  ...
  <div class="index">
    ...
    <div class="index-main">
      <div
        ...
        :style="{transform: `matrix( ${scale.x}, 0, 0, ${scale.y}, ${translate.x}, ${translate.y} )`}"
      >
        <img :src="mainSrc" />
...
export default {
  ...
  data() {
    return {
      ...
      preData: window.localStorage.getItem("preData") || ""
    };
  },
  computed: {
    toolColor() {
  ...
  methods: {
    ...
    bindEvent() {
      ...
      hammer.on("pinchend", e => {
        ...
        if (
          this.scale.x - this.lockScale.x.min <
          this.lockScale.x.min * mixMoveMin
        ) {
          this.translate.x = 0;
          this.translate.y = 0;
          this.scale.x = this.lockScale.x.min;
        ...
        if (this.scale.x >= this.lockScale.x.max - mixMoveMax) {
          ...
          this.scale.y =
            this.lockScale.y.max -
            (this.lockScale.y.min / this.lockScale.x.min) * mixMoveMax;
          this.baseScaleToTranslate();
        }
      });
    ...
    baseScaleToTranslate() {
      ...
      // 比例相等原则，两个线段比例相等，则他们前后对应的变化也必须要相等
      let TChange = {
    ...
    save() {
      ...
      console.log(data);
      window.localStorage.setItem("preData", data.canvasJson);
    }
  },
  components: {
    canvasShow
  ...
  &-header {
    ...
    &-right {
      button {
        ...
        border-radius: 5px;
        color: #aaaaaa;
        background: url("../../assets/img/footer/back.png") no-repeat left
          center;
      }
      .canBack {
        color: #07c160;
        background: url("../../assets/img/footer/can-back.png") no-repeat left
          center;
      }
    }
    &-title {
